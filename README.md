...
```bash
tmp="$(mktemp -d)"
git clone --depth 1 --branch v0.9.5 https://github.com/hjung3113/general-low-reasoning-agent-harness.git "$tmp"
...
| 하네스 업그레이드 | remembered init scope | `python3 scripts/upgrade_harness.py --version v0.9.5 --dry-run` |
...
python3 scripts/harness.py check --worktree
python3 scripts/release_smoke_test.py
python3 scripts/harness.py release-check --expected-version v0.9.5
