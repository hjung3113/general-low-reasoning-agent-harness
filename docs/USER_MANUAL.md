...
```bash
git -c gpg.ssh.allowedSignersFile=docs/trust/allowed-signers verify-tag v0.9.6
```
...
1. 터미널에서 `python3 scripts/harness.py release v0.9.6` 실행
2. 하네스가 묻습니다: `Type 'release v0.9.6' to confirm:`
3. 정확히 `release v0.9.6`를 타이핑하고 Enter
...
| `release-check --expected-version v0.9.6` | Release tag 버전 검증 |
...
```bash
python3 scripts/upgrade_harness.py --source /path/to/newer-harness --version v0.9.6 --dry-run
```
...
git clone --depth 1 --branch v0.9.6 https://github.com/hjung3113/general-low-reasoning-agent-harness.git $tmp.FullName
