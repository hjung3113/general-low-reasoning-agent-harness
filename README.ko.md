...
| `.planning/milestones/NN-*/` | 타겟 소유 | 마일스톤별 planning 문서 |
...
신규 설치 직후 `harness check` 가 skeleton phase (`00-orientation`) 관련 경고를 낼 수 있음. 정상 — skeleton 이 시드한 템플릿 planning 파일이 첫 마일스톤 선언 시 교체되도록 설계됨. `.planning/ROADMAP.md` 에 bullet 추가하고 `.planning/STATE.md` 에 stamp 하면 `harness check` 가 조용해짐.
...
작업 마일스톤 = GitHub milestone + issue 로 관리 (`hjung3113/general-low-reasoning-agent-harness`). [`docs/agents/issue-tracker.md`](docs/agents/issue-tracker.md), [`docs/agents/triage-labels.md`](docs/agents/triage-labels.md) 참조. 마일스톤별 plan: [`.planning/milestones/NN-*/`](.planning/milestones/).
