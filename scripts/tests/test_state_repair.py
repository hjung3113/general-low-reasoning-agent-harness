...
class CanonicalRenderTests(unittest.TestCase):
    def test_canonical_roadmap_phases_payload_renders_checkbox_list(self):
        ...
        self.assertEqual(
            payload,
            "- [x] **Milestone 0: Planning Hydration**\n"
            "- [ ] **Milestone 1: Schema Module**\n",
        )
    ...
    def test_canonical_state_current_payload_renders_section(self):
        payload = canonical_state_current_payload(
            ...
            checkpoint="CP-01-01",
            checkpoint_path=".planning/milestones/01-schema/01-CHECKPOINTS.md",
        )
        self.assertIn("- **Milestone**: 1 - Schema Module", payload)
        self.assertIn("- **Checkpoint**: CP-01-01", payload)
        self.assertIn(
            "- **Checkpoint file**: `.planning/milestones/01-schema/01-CHECKPOINTS.md`",
            payload,
        )
