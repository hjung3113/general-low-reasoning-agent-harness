...
10. `test_golden_contains_four_lifecycle_audit_entries` — load golden; assert it has an `audit_entries` array of length 5 (one `phase.set discuss`, one `phase.set plan`, one `phase.approve`, one `phase.set execute`, one `phase.set done`) with `verb` values matching ADR Artifact 1 verb names exactly.
