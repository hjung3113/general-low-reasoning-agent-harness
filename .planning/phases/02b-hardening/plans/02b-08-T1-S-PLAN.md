...
Spec: `docs/superpowers/specs/2026-05-16-hardening-slice-design.md` §7 T1-S row (size bumped to **M** per ADR-bundle revisor — actual file count is 17, not the 3–5 placeholder), §10.2 (static grep gate), §10.3 (quarantine rationale), §9.1 (Haiku-4.5 trial harness this row feeds).
...
**Note A — `.roo/commands/done.md` absence:** The 4-lifecycle-commands-per-adapter rule in spec §10.2 expects `discuss`, `plan`, `execute`, `done` in `.roo/commands/`. Roo currently ships only the first three as `phase-*.md`; there is no `.roo/commands/done.md` and the closure step is handled inside `phase-execute.md`'s "if verification passes, move to done" branch. T1-S does NOT create `.roo/commands/done.md` (that is a parity gap owned by T0-3 adapter mirroring, per spec §8 "Adapter mirroring is part of T0-3 and T1-1, not a separate row"). T1-S records the gap in the slice summary and proceeds with 3 Roo lifecycle files + 4 OpenCode lifecycle files = 7 adapter files touched.
...
- **Q5 — V8–V11 trial count (5 vs 50).** Spec §9.1 mandates N=50 per flow for the slice-level acceptance. T1-S uses N=5 per flow as a per-row preview because the full scenario runner does not yet exist and the per-trial cost (5 × 4 flows × manual paste = ~30 minutes) is a hard ceiling for a row-level smoke. The full N=50 run lands in the dedicated §9.1 harness task and consumes the SKILL surface this row produces. The reviewer is asked to confirm the row-level 5-trial preview is an acceptable risk mitigation.
